...
Ce pipeline, entièrement développé en interne (homemade), couvre l'ensemble du cycle de livraison logicielle. Lorsqu'un développeur pousse du code sur la branche \texttt{develop}, la CI s'exécute automatiquement~: lint, tests, build des images multi-architecture (linux/amd64 et linux/arm64), push vers le GitHub Container Registry, scan de sécurité Trivy, puis déclenchement du webhook Dokploy qui redéploie la stack de pré-production avec les images \texttt{:develop}. Le même processus s'applique à la branche \texttt{main} pour la production avec les images \texttt{:latest}.

L'ensemble du processus, du push Git au déploiement effectif, prend environ 5 minutes et ne nécessite aucune intervention manuelle. Cette automatisation complète permet à l'équipe de livrer rapidement les correctifs et les nouvelles fonctionnalités, tout en garantissant que chaque livraison passe par les mêmes étapes de validation.
...
builds successifs. Cette automatisation permet de valider et livrer du code en production de manière fiable et reproductible, sans intervention manuelle.
...
L'ensemble de la pipeline est entièrement automatisée~: du push Git au déploiement en production en environ 5 minutes, sans aucune intervention manuelle.

Dokploy est exécuté dans un conteneur LXC dédié sur le serveur Proxmox.
La stratégie de déploiement est la suivante~: un push sur la branche
\texttt{main} construit les images avec le tag \texttt{:latest} et déploie
la stack de production~; un push sur la branche \texttt{develop} construit
les images avec le tag \texttt{:develop} et déploie la stack de
pré-production. Dokploy surveille les appels webhook provenant de GitHub
Actions et redéploie automatiquement la stack concernée. Ce mécanisme
permet un workflow \textbf{GitOps} complet~: le code est poussé, la CI
construit les images, et la CD les déploie sans intervention manuelle.
...
Les cibles de scraping utilisent les noms de services Docker Compose
(\texttt{edr-server}, \texttt{site-web}) plutôt que les noms de conteneurs
(\texttt{lockbits\_edr}, \texttt{lockbits\_web}). L'utilisation du \texttt{project name}
(\texttt{lockbits-prod}) garantit que les noms DNS pointent vers la bonne stack,
évitant le round-robin DNS entre production et pré-production.
