...
Le serveur est containerise via un Dockerfile multi-étapes. L'image est
...
L'architecture est composée de quatre modules :
...
fichiers exécutables (.exe, .dll, .bat, .ps1, .sh) et les chemins
...
Les binaires sont distribués via les GitHub Releases du dépôt principal.
...
et \texttt{--uninstall} (désinstallation complète).
...
visualiser leur tableau de bord, gérer leurs tickets de support, et
...
commande d'installation pré-remplie. L'interface est construite avec
...
définies dans \texttt{client/database.sql} :
...
\begin{itemize}
  ...
  \item \texttt{tickets} -- tickets de support relies aux utilisateurs
    et à GLPI (\texttt{glpi\_ticket\_id}) ;
  \item \texttt{ticket\_messages} -- messages associés aux tickets ;
...
basée sur \texttt{php:8.2-apache}. Elle intégre :
...
Le chatbot est implementé dans un fichier PHP unique
...
\begin{itemize}
  ...
  \item Fournir un résumé du tableau de bord client (tentatives DDoS
    bloquées, vulnérabilités, incidents) ;
...
\begin{itemize}
  \item \textbf{Rate limiting} -- compteur quotidien base sur un fichier
    ...
    sources confondues. Le compteur est réinitialisé chaque jour ;
...
L'interface utilisateur du chatbot est integrée directement dans le
dashboard client via du JavaScript vanilla. Elle affiche un badge de
notification en cas de nouveau message non lu et propose une expérience
