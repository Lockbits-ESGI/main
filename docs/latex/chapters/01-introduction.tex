...
Informatique) de Paris. Le projet fil rouge se déroule sur l'année
...
agent déployé sur chaque machine à protéger. Le portail client complète
l'offre en offrant une interface web de gestion, de visualisation et de
support.
...
Sur le plan de la disponibilité, le service EDR est critique pour les
clients. Une interruption de la détection expose l'entreprise à des
risques de sécurité. Le système doit donc être résilient et permettre
...
\begin{enumerate}
  ...
  \item \textbf{Site client web}~: réaliser un portail client en
    ...
    abonnés et système de tickets de support.
...
conception d'architecture, administration système et gestion de bases de
données.

La gestion de projet occupe une place centrale. L'équipe doit organiser
son travail, répartir les tâches, tenir des réunions hebdomadaires et
respecter des sprints de deux semaines. Cette organisation reproduit les
méthodes agiles utilisées en entreprise.
...
\begin{itemize}
  ...
  \item \textbf{Déploiement}~: Docker Compose en production, avec
    Dokploy comme plateforme d'hébergement.
...
de septembre à avril. Les composants sont développés en parallèle via
des sous-modules Git, chaque sous-module étant un dépôt indépendant avec
son propre cycle de développement.

Le travail est organisé en sprints de deux semaines, avec des réunions
hebdomadaires de suivi. Cette organisation permet de détecter
rapidement les blocages et d'ajuster les priorités.
